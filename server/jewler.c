...
void attempt_do_jewler(object *caster, object *cauldron) {
    ...
    // Start resistances 
        ...
        if(base_item->type == AMULET)
        {
            ...
            merge_item = object_find_by_type(cauldron, AMULET);
            if (merge_item == NULL) { /* failure--no type found */
                base_item->type = AMULET; // set it back
        ...
        {
            base_item->type = NULL; // set out base item to null temp so we can search again by type.
            merge_item = object_find_by_type(cauldron, RING);
            if (merge_item == NULL) { /* failure--no type found */
                base_item->type = RING; // set it back
                draw_ext_info(NDI_UNIQUE, 0, caster, MSG_TYPE_SKILL, MSG_TYPE_SKILL_FAILURE,
                        "You need two base items of the same type to merge.");
                return;
            }
